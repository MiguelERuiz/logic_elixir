defmodule UnificationTest do
  ...
  test "Verifies [Var1], [Var2], [Occurs-check] and [Orient] rules" do
    ...
    assert unify({:var, "Y"}, [{:var, "X"}, {:var, "Z"} | {:ground, []}], %{}) == %{
             "Y" => [{:var, "X"}, {:var, "Z"} | {:ground, []}]
           }

    assert unify({:var, "X"}, {:ground, [1, 2]}, %{}) == %{"X" => {:ground, [1, 2]}}
  end
  ...
  test "Verifies [List] rule" do
    ...
    assert unify([{:var, "X"}, {:var, "X"} | {:ground, []}], {:ground, [5, 5]}, %{}) == %{
             "X" => {:ground, 5}
           }
    ...
    assert unify(
             [{:var, "Y"}, {:var, "X"}, {:var, "Z"} | {:ground, []}],
             [{{:var, "X"}, {:ground, 3}} | {:ground, [5, :a]}],
             %{}
